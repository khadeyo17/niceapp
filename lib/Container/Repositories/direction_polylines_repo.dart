...
import 'package:niceapp/Model/direction_polyline_details_model.dart';
...
final globalDirectionPolylinesRepoProvider = Provider<DirectionPolylines>((
  ref,
) {
...
class DirectionPolylines {
  ...
  void setNewDirectionPolylines(ref, context, controller) async {
    try {
      DirectionPolylineDetails model = await getDirectionsPolylines(
        context,
        ref,
      );
      ...
      List<PointLatLng> decodedPolylines = points.decodePolyline(
        model.epoints!,
      );
      ...
      if (decodedPolylines.isNotEmpty) {
        for (PointLatLng polyline in decodedPolylines) {
          pLinesCoordinatedList.add(
            LatLng(polyline.latitude, polyline.longitude),
          );
        }
      }
